...
function countLetterA(arr) {
  var arr1 = arr.split(``);
  var arr2 = arr1.filter(function(a){return ['a'].indexOf(a)>-1;}).length;
  return arr2;
}
...
function reverseEachWord(array, boolen) {
  var arr1 = array.split(` `).map(function(elem, index, arr){
    var arr2 = elem.split(``).reverse().join(``);
    return arr2;    
  })
  if (boolen == true) {
    arr1.reverse();
...
let counter = sentence.split(` `).map(function(elem, index, arr){
...
function createHashTags(arr) {
  var company1 = listOfCompanys.map(function(elem, index, arr){
 var arr2 = elem._id;
